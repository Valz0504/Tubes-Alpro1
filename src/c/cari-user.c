...
void printUserTableRow(User *u) {
    printf("| %-3d | %-20s | %-9s | %-20s |\n",
        ...
        u->role == ROLE_PASIEN ? u->riwayatPenyakit : "-"
    );
}
...
void cariUser(UserList *dataBaseUser, User *currentUser, boolean *isLogin) {
    if (!(*isLogin)) {
    ...
    if (currentUser->role == ROLE_MANAGER) {
        ...
        switch (pilihan) {
            case 1: {        // cari berdasarkan ID
                ...
                u = findUserByID(dataBaseUser, nomorId);
                if (u == NULL) {
...
void cariPasien(UserList *dataBaseUser, User *currentUser, boolean *isLogin) {
    if (!(*isLogin)) {
        printf(RED "Login sebagai Manager terlebih dahulu!\n\n" RESET);
        return;
    }
                        ...
                        switch (urut) {
                            case 1:
                                ...
                                for (int i = 0; i < dataBaseUser->Neff; i++) {
                                    if (dataBaseUser->data[i].role == ROLE_PASIEN && strcmp(dataBaseUser->data[i].riwayatPenyakit, penyakit) == 0) {
                                        found = TRUE;
                        ...
                        switch (urut) {
                            case 1:
                                ...
                                for (int i = 0; i < temp.Neff; i++) {
                                    if (temp.data[i].role == ROLE_PASIEN && strcmp(temp.data[i].riwayatPenyakit, penyakit) == 0) {
                                        found = TRUE;
...
void cariDokter(UserList *dataBaseUser, User *currentUser, boolean *isLogin) {
    if (!(*isLogin)) {
        printf(RED "Login sebagai Manager terlebih dahulu!\n\n" RESET);
        return;
    }
